...
public:
	...
	inline bool diff (string cur, string word) {
		int d = 0;
	...
	inline vector<string> get_close (string cur, unordered_set<string>& wordList, unordered_map<string, vector<string>>& close_dict) {
		if (close_dict.find(cur) != close_dict.end()) return close_dict[cur];
	...
	vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
		...
		unordered_map<string, vector<string>> close_dict;
		...
		while (!q.empty()) {
			...
			q.pop();
			vector<string> close_words = get_close(cur, wl, close_dict);
