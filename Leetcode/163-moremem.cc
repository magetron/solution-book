...
public:
    vector<string> findMissingRanges(vector<int>& nums, int lower, int upper) {
		vector<string> ans;
		if (nums.size() == 0) {
			if ((long)upper - (long)lower > 0) ans.push_back(to_string(lower) + "->" + to_string(upper));
		...
		//cout << l << " " << r << endl;
		if ((long)nums[l] - (long)lower > 0) {
			if ((long)nums[l] - (long)lower == 1) ans.push_back(to_string(lower));
		...
		if ((long)upper - (long)nums[r] > 0) {
			if ((long)upper - (long)nums[r] == 1) ans.push_back(to_string(upper));
